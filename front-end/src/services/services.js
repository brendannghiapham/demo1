...
export const fetchKpiData = async (selectedProjects, startDate, endDate) => {
    ...
    const projectQuery = selectedProjects.map(p => p.value).join(',');
...
export const fetchSprintVelocity = async (selectedProjects) => {
    ...
    try {
        const response = await axios.get(`${API_BASE_URL}/sprint-velocity`, { params: { projects: projectQuery } });
        return response.data || [];
    } catch (error) {
...
export const fetchBugData = async (selectedProjects) => {
    ...
    try {
        const response = await axios.get(`${API_BASE_URL}/bugs`, { params: { projects: projectQuery } });
        return response.data || {};
    } catch (error) {
