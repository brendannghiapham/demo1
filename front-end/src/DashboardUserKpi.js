...
import { getAllUserProjects } from './utils/collection.utils';
...
const DashboardUserKpi = () => {
  ...
  useEffect(() => {
    axios
      ...
      .then((response) => {
        const projects = getAllUserProjects(response.data);
        console.log('[DahsboardUserKpi] , projects: ', projects);
        ...
        <TableBody>
          {userData.map((user) => (
            <TableRow key={user.assignee}>
              ...
              <TableCell sx={{ width: columnWidths.issuesCount }}>
                <Box display="flex" flexDirection="column" alignItems="center">
                  ...
                  <Typography variant="body2">
                    {user.done_issues_percentage.toFixed(2)}% Done
                  </Typography>
                </Box>
              </TableCell>
                ...
                <Box display="flex" flexDirection="column" alignItems="center">
                  <LinearProgress
                    ...
                    value={calculateRemainingPercentage(
                      user.remaining_days,
                      user.total_estimated_time
                    )}
                    color={getProgressBarColor(
                      calculateRemainingPercentage(user.remaining_days, user.total_estimated_time)
                    )}
                    sx={{ width: '100%', height: progressBarHeight, marginTop: 1 }}
                  />
              ...
              <TableCell sx={{ width: columnWidths.projects }}>
                {user.projects.map((el) => el.key).join(', ')}
              </TableCell>
                    ...
                    {user.issue_types
                      ...
                      .map((type) => (
                        ...
                        >
                          ...
                          <Typography variant="body2">
                            {type.total_remaining_time.toFixed(2)} days
                          </Typography>
                        </Box>
                      ))}
                  </>
